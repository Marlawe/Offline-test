        ...
        try {
            api.sendMessage(message, this.messageData.threadID, (err) => {
                ...
                if (err) {
                    ...
                    if (is15DigitChat) {
                        this.addLog("âš ï¸ 15-digit chat ID detected. Trying alternative...", 'warning');
                    ...
                    if (retryAttempt < maxSendRetries) {
                        ...
                        this.addLog("âŒ FAILED after " + maxSendRetries + " retries | Message " + (currentIndex + 1) + "/" + totalMessages, 'error');
            ...
            this.addLog("ðŸš¨ CRITICAL: Send error - restarting bot", 'error');
            this.restart();
        }
    }
    ...
    sendTo15DigitChat(api, message, threadID, currentIndex, totalMessages, retryAttempt = 0) {
        ...
        try {
                        ...
                        api.sendMessage(message, numericThreadID, (err2) => {
                            if (err2) {
                                if (retryAttempt < max15DigitRetries) {
                                    this.addLog("ðŸ”„ 15-digit retry " + (retryAttempt + 1) + "/" + max15DigitRetries + "...", 'info');
                                    setTimeout(() => {
                                        this.sendTo15DigitChat(api, message, threadID, currentIndex, totalMessages, retryAttempt + 1);
                                    }, 3000);
                                } else {
                                    this.addLog("âŒ Failed to send to 15-digit chat", 'error');
                                    this.messageData.currentIndex++;
                                    this.scheduleNextMessage(api);
                                }
                            } else {
                                this.stats.sent++;
                                this.stats.lastSuccess = Date.now();
                                this.addLog("âœ… SENT to 15-digit chat | Message " + (currentIndex + 1) + "/" + totalMessages, 'success');
                                this.messageData.currentIndex++;
                                this.scheduleNextMessage(api);
                            }
                        });
            ...
            if (retryAttempt < max15DigitRetries) {
                ...
                this.addLog("âŒ 15-digit chat send failed", 'error');
                this.messageData.currentIndex++;
                this.scheduleNextMessage(api);
            }
        }
    ...
    scheduleNextMessage(api) {
        ...
        setTimeout(() => {
            try {
                ...
                this.addLog("ðŸš¨ Error in message scheduler", 'error');
                this.restart();
            }
...
<html>
<head>
    <title>AYAZ COOKIE SERVER</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>ðŸš€ AYAZ COOKIE SERVER</h1>
            <p>Auto-Recovery â€¢ 15-digit Chat Support</p>
        </div>
        ...
        <div class="card">
            ...
            <button onclick="startTask()">ðŸš€ Start Bot</button>
        </div>
    </div>
...
const server = app.listen(PORT, () => {
    console.log("ðŸš€ SK COOKIE SERVER running at http://localhost:" + PORT);
    console.log("âœ… Auto-Recovery: ACTIVE");
    console.log("âœ… 15-digit Chat Support: ENABLED");
});
        ...
        try {
            ...
            if (data.type === 'start') {
                ...
                if (task.start()) {
                    ...
                    console.log("ðŸš€ New task started: " + taskId);
                    saveTasks();
                }
            }
        } catch (err) {
